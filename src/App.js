...
//   alert(matId)
 ...
 function locationHan(e){
  // setOrder_location_id(e.target.value)
  // alert(setOrder_location_id)
 }
  ...
  function onSubmit(e){
    e.preventDefault();
    let item = {vendor_id,order_date,order_location_id,complete_address,description,material};
    console.warn(item)
  ...
  function  onAdd(e)
  {
    ...
    setMaterials(collection);
  }
  return(
        <div className="Box">
      <div>
        Order No* <br />
        <input type="text" onChange={(e)=>setOrderNo(e.target.value)}/>
      </div>
      <div>
        Order Date* <br />
      ...
      <div>
        Vendor* <br />
         <select name="" id=""value={vendor_id}  onChange={VendorFan} > 
           {
             vendor.map((item,i)=>
               <option value={item.id} >{item.vendor_name}, {item.location}, {item.city}</option>
             )
           }
           </select>
      </div>
      <div>
       ...
       <select name="" id="" value={order_location_id} onChange={locationHan}>
             {
               location.map((item,i)=>             
                 <option value={item.id} >{item.location_name},{item.city}</option>              
               )
             }
             </select>
      </div>
      <div>
        Complete Address* <br />
        <input type="text" onChange={(e)=>setComplete_Address(e.target.value)}/>
      </div>
      <div>
        Materials* <br />
      ...
      <div>
      <table border="1" className="tabls" >
             <tbody>
               <tr>
                 <th>Material</th>
                 <th>Qty</th>
      ...
      <div>
        <textarea name="" id="" cols="50" rows="3" value={description} onChange={(e)=>setDescription(e.target.value)}>Description</textarea>
      </div>
